<script lang="ts">
    ...
    function handleLogin(event: SubmitEvent): void {
        ...
        if (login === 'user' && password === 'user') {
            currentRole = 'user';
            currentAccountLabel = 'user';
            authMessage = 'Zalogowano na konto user (limit kredytu: 100000).';
            errors = [];
            return;
        }

        if (login === 'szef' && password === 'szef') {
            currentRole = 'szef';
            currentAccountLabel = 'szef';
            authMessage = 'Zalogowano na konto szef (brak limitu kredytu).';
            errors = [];
            return;
        }
        ...
        currentRole = null;
        currentAccountLabel = '';
        authMessage = 'Nieprawidlowy login lub haslo.';
